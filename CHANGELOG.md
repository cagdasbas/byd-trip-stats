...
- **Range projection now reports a realistic value from the first kilometre instead of being pinned in `● Baseline`** — on Seal Excellence (and any car with a sparse or unreliable engine-power signal) the projection used to sit on the catalog baseline for the entire drive, sometimes underestimating range by half. Several things changed together: (a) the rolling-window energy accumulator and the speed-bin accumulator both feed from the BMS `totalDischarge` delta — the same source the CONS readout uses — with engine-power only as a fallback, both during a live drive and when restoring an in-progress trip from disk; (b) the speed-bin accumulator now counts distance every tick, with energy added only when positive, so sparse BMS updates can't starve the bin distance below `BIN_MIN_DIST_KM` (was the dominant Seal Excellence failure mode — bin distance never reached 0.2 km and `binWhPerKm` stayed null); (c) the `● Historical bins` tier averages Wh/km across *all* populated speed bins weighted by distance and only requires 0.2 km of total drive to engage (was 0.5 km inside the *current* bin alone, which reset on every speed-bin transition); (d) the model badge is picked across all three tiers (`● Live trip` / `● Speed bins` / `● Baseline`) when the app is reopened mid-trip instead of skipping `● Speed bins` entirely; (e) the orange projection line is drawn from km 0 in the restored chart rather than leaving a 3 km gap before the old stabilisation threshold. End result on a typical city drive: the badge flips from `● Baseline` to `● Speed bins` within ~0.2 km, then to `● Live trip` at ~3 km, and the projection converges to roughly `(remaining kWh × 100) / CONS` from the first kilometre instead of after 30+ km of driving.
